...
import {isSelected} from "./ButtonN"
...
export type BubbleTextFieldAttrs<T> = {
	label: TranslationKey | lazy<string>;
	suggestions: string;
	bubbleHandler: BubbleHandlerN<T, Suggestion>;
}
 ...
 */
export interface BubbleHandlerN<T, S:Suggestion> {
	/**
...
export class _BubbleTextField<T> {

	view(a: Vnode<BubbleTextFieldAttrs<T>>) {
			...
			m(TextFieldN, {
				...
				oncreate() {
					...
					requestAnimationFrame(() => {
						if (this._domInput) this._domInput.size = 1
					})
				}
			}),
...
export const BubbleTextFieldN: Class<MComponent<BubbleTextFieldAttrs<T>>> = _BubbleTextField<T>